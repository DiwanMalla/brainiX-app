      ...
      try {
        ...
        if (error.message.includes("400") && error.failed_generation) {
          ...
          try {
            ...
            console.log("Parsed failed_generation:", failedResponse); // Debug: Log parsed failed_generation
            if (Array.isArray(failedResponse.suggestions)) {
              suggestionsArray = failedResponse.suggestions
                .filter((item: any) => item.id && item.text) // Ensure id and text exist
                .map((item: any) => ({
                  id:
                    item.id.toString() ||
                    Math.random().toString(36).substring(2, 15),
                  text: item.text || "unknown", // Fallback text if missing
                }));
              errorMessage =
