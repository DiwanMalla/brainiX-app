...
export default function CourseLearningScreen() {
  ...
  const markLessonComplete = async (
    ...
    if (!course || !course.modules[moduleIndex]?.lessons[lessonIndex]) return;
    ...
    try {
      ...
      setCourse((prevCourse) => {
        ...
        const newCourse = { ...prevCourse, modules: updatedModules };
        const totalLessons = newCourse.modules.reduce(
          (sum, module) => sum + module.lessons.length,
          0
        );
        const completedLessons = newCourse.modules.reduce(
          (sum, module) =>
            sum +
