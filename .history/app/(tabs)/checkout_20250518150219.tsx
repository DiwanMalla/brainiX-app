...
export default function CheckoutScreen() {
  ...
  const validateBillingDetails = (requireCard: boolean = false): FormErrors => {
    ...
    // Require card details only when explicitly needed (e.g., on payment)
    if (requireCard) {
  ...
  // New function to validate and fetch client secret manually
  const validateAndFetchClientSecret = async () => {
    ...
    const errors = validateBillingDetails(true); // Require card details
  ...
  const handlePayment = async () => {
    ...
    // Fetch client secret if not already set
    if (!clientSecret) {
      await fetchClientSecret(billingDetails);
      if (!clientSecret) return; // Exit if fetch fails
    }
  ...
  // Monitor billing details to trigger validation after all fields are filled
  useEffect(() => {
    if (!cartItems.length || !user || isLoading) return;

    const errors = validateBillingDetails(false); // Don't require card yet
    setFormErrors(errors);
          ...
          {paymentSuccess ? (
            ...
            <>
              ...
              <TouchableOpacity
                ...
                onPress={validateAndFetchClientSecret}
