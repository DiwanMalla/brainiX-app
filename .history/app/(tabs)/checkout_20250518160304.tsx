...
export default function CheckoutScreen() {
    ...
    const fetchClientSecret = async () => {
      ...
      try {
        ...
        if (err && typeof err === "object" && "message" in err) {
          console.error("fetchClientSecret error:", (err as { message?: string }).message);
        } else {
          console.error("fetchClientSecret error:", String(err));
        }
  ...
  const applyPromoCode = async () => {
    ...
    try {
      ...
      setError(
        err && typeof err === "object" && "message" in err
          ? (err as { message?: string }).message || "Failed to validate promo code"
          : String(err)
      );
      Toast.show({
        type: "error",
  ...
  const handlePayment = async () => {
    ...
    try {
      ...
      console.log("Confirm payment result:", { paymentIntent, stripeError });
      ...
      if (paymentIntent) {
        ...
        if (status === "succeeded") {
          ...
          if (!orderNumber) {
            console.error("Order number missing in response");
          ...
          console.log("Navigating to:", `/thankyou?orderNumber=${orderNumber}`);
          router.push(`/thankyou?orderNumber=${orderNumber}`);
        } else {
          console.log(
          ...
          Toast.show({
            ...
            text2: `Payment status: ${paymentIntent.status}`,
          });
        }
      } else {
        ...
        Toast.show({
          type: "error",
          text1: "Payment Error",
          text2: "No payment intent returned.",
        });
      }
    } catch (err: any) {
      console.log("Payment error:", err.message, err.stack);
      setError(err.message || "Unexpected error during payment");
      Toast.show({
        type: "error",
        text1: "Error",
        text2: err.message || "Something went wrong during payment.",
      });
    } finally {
      setPaymentLoading(false);
      console.log("Payment process completed, loading:", false);
    }
  };
