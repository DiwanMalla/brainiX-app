...
const groq = new Groq({
  apiKey: "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",
});
  ...
  useEffect(() => {
    const fetchSuggestions = async () => {
      ...
      try {
        ...
        const prompt = `Generate a JSON object containing an array of up to 5 keyword or tag suggestions relevant to the search query "${searchQuery}" for an e-learning platform. Each suggestion should be a single word or short phrase (e.g., "web development", "python", "data analysis") related to the courses provided, but not the course titles or category names themselves. Use the following course data: ${JSON.stringify(
          allCourses.map((c) => ({
            title: c.title,
            description: c.shortDescription,
            category: c.category.name,
          }))
        )}. Return the response in this exact JSON format: {"suggestions": [{"id": "1", "text": "keyword1"}, {"id": "2", "text": "keyword2"}, ...]}. Ensure all suggestions are relevant to the query.`;
