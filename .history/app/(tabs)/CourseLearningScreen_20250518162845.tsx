...
import { Tabs, TabsList, TabsTrigger, TabsContent } from "@rnx-kit/react-native-tabs";
import { Ionicons } from "@expo/vector-icons";
import { debounce } from "lodash";
import VideoPlayer from "@/components/my-learning/VideoPlayer"; // Adjust path based on your file structure
...
interface ContentItem {
  ...
  progress?: Array<{ completed: boolean; watchedSeconds?: number; lastPosition?: number }>;
}
...
const CourseLearningScreen = () => {
  ...
  const [noteInput, setNoteInput] = useState(""); // Added for Notes panel
      ...
      try {
        ...
        const completedLessons = res.data.modules.reduce(
          (sum: number, module: Module) =>
            sum + module.lessons.filter((lesson: ContentItem) => lesson.progress?.[0]?.completed).length,
          0
        );
  ...
  const markLessonComplete = async () => {
    ...
    try {
      ...
      setProgress(totalLessons > 0 ? ((completedLessons + 1) / totalLessons) * 100 : 0);
  ...
  const handleVideoProgress = debounce(
    async (state: { playedSeconds: number; played: number }) => {
      ...
      try {
              ...
              lIdx === activeLesson
                ? {
                    ...
                    progress: lesson.progress?.length
                      ? [{ ...lesson.progress[0], watchedSeconds, lastPosition: watchedSeconds }]
        ...
        setError(err.response?.data?.message || "Failed to update video progress");
      }
    },
    15000
  );
  ...
  const handleAddNote = async (content: string) => {
    if (!content.trim() || !course || !course.modules[activeModule]?.lessons[activeLesson]) return;
  ...
  if (loading) {
    return <ActivityIndicator style={styles.center} size="large" color="#3b82f6" />;
  }
  ...
  if (!course || course.modules.length === 0) {
    return (
      <View style={styles.center}>
        <Text style={styles.emptyText}>No content available for this course.</Text>
      </View>
    );
  }

  const currentLesson = course.modules[activeModule]?.lessons[activeLesson];
  const lessonNotes = notes.filter((note) => note.lessonId === currentLesson?.id);
          ...
          <View style={styles.contentContainer}>
            ...
            <Tabs defaultValue="content" style={styles.tabs}>
              <TabsList style={styles.tabsList}>
                <TabsTrigger value="content" title="Content" style={styles.tabTrigger} />
                <TabsTrigger value="resources" title="Resources" style={styles.tabTrigger} />
                <TabsTrigger value="transcript" title="Transcript" style={styles.tabTrigger} />
                <TabsTrigger value="ai-quiz" title="AI Quiz" style={styles.tabTrigger} />
              </TabsList>

              <TabsContent value="content" style={styles.tabContent}>
                <Text style={styles.tabContentText}>
                  {currentLesson?.description || "No description available."}
                </Text>
              </TabsContent>
              <TabsContent value="resources" style={styles.tabContent}>
                <Text style={styles.tabContentText}>
                  No resources available for this lesson.
                </Text>
              </TabsContent>
              <TabsContent value="transcript" style={styles.tabContent}>
                <Text style={styles.tabContentText}>
                  Transcript not available for this lesson.
                </Text>
              </TabsContent>
              <TabsContent value="ai-quiz" style={styles.tabContent}>
                <Text style={styles.tabContentText}>
            ...
            <View style={styles.navigationButtons}>
              <TouchableOpacity
                style={[styles.navButton, { opacity: activeLesson === 0 && activeModule === 0 ? 0.5 : 1 }]}
              ...
              <TouchableOpacity
                style={[styles.navButton, { opacity: currentLesson?.progress?.[0]?.completed ? 0.5 : 1 }]}
                disabled={currentLesson?.progress?.[0]?.completed}
                onPress={markLessonComplete}
              >
                <Text style={styles.navButtonText}>
                  {currentLesson?.progress?.[0]?.completed ? "Completed" : "Mark as Complete"}
                </Text>
              </TouchableOpacity>
              <TouchableOpacity
                style={[
      ...
      {showNotes && (
        <View style={styles.panel}>
          ...
          <TouchableOpacity
            ...
            onPress={() => handleAddNote(noteInput)}
          >
          ...
          renderItem={({ item: module, index: moduleIndex }) => (
            <View style={styles.moduleContainer}>
              <View style={styles.moduleHeader}>
                ...
                {module.lessons.every((lesson) => lesson.progress?.[0]?.completed) && (
...
const styles = StyleSheet.create({
  ...
  lessonTitle: { fontSize: 18, fontWeight: "600", color: "#1f2937", marginBottom: 8 },
  tabs: { marginTop: 12 },
  tabsList: {
    flexDirection: "row",
  ...
  noteCard: {
    ...
    marginHorizontal: 16,
    marginBottom: 8,
    backgroundColor: "#front-end-developer",
    borderRadius: 8,
  },
  ...
  chatContainer: { maxHeight: 300, padding: 16 },
    ...
    messageTime: { fontSize: 12, color: "#6b7280", marginLeft: 8 },
    instructorBadge: {
    ...
    instructorBadgeText: { fontSize: 10, color: "#fff", fontWeight: "bold" },
    messageContent: { fontSize: 14, color: "#374151", marginTop: 2 },
    messageActions: { flexDirection: "row", marginTop: 4 },
    messageActionButton: { flexDirection: "row", alignItems: "center", marginRight: 12 },
    messageActionText: { fontSize: 12, color: "#6b7280", marginLeft: 4 },
    chatInputContainer: {
      flexDirection: "row",
      alignItems: "center",
      padding: 12,
    ...
    sidebarContent: { paddingBottom: 100 },
    sidebarTitle: { fontSize: 18, fontWeight: "bold", color: "#1f2937", marginLeft: 16 },
    sidebarSubtitle: { fontSize: 14, color: "#6b7280", marginLeft: 16,