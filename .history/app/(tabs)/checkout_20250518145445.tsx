...
export default function CheckoutScreen() {
  ...
  const validateBillingDetails = (): FormErrors => {
    ...
    // Make state optional for countries without states (e.g., UK, CA)
    if ((country === "US" || country === "AU") && !state)
      errors.state = "State is required";
    if (!postalCode) errors.postal_code = "Postal code is required";
    if (!country) errors.country = "Country is required";
    // Relax card number validation to allow partial input during typing
    if (cardNumber && cardNumber.replace(/\s/g, "").length !== 16)
      errors.cardNumber = "Valid 16-digit card number is required";
    // Accept MM/YY or MM/YYYY for expiry
  ...
  const handlePayment = async () => {
    ...
    const billingDetails: BillingDetails = {
      ...
      address: {
        line1: address,
        city,
        state: state || "", // Ensure state is sent even if empty
    ...
    try {
      ...
      // Normalize expiry for Stripe (convert MM/YYYY to MM/YY)
  ...
  useEffect(() => {
    ...
    if (Object.keys(errors).length === 0) {
      console.log("Fetching client secret with:", billingDetails); // Debug: Log before fetch
        ...
        <View style={styles.card}>
          ...
          {paymentSuccess ? (
            ...
            <View style={styles.loadingContainer}>
              <ActivityIndicator size="small" color="#a500ff" />
              <ThemedText style={styles.loadingText}>
                Preparing payment...
              </ThemedText>
            </View>
          )}
          {/* Display all validation errors if clientSecret is null */}
          {Object.keys(formErrors).length > 0 && !clientSecret && (
            <Animated.View
              entering={FadeIn}
              exiting={FadeOut}
              style={styles.messageContainer}
