    ...
    async function fetchMessages() {
      ...
      try {
        ...
        if (isMounted && fetchId === fetchIdRef.current) {
          ...
          console.log("fetchMessages: Ignored outdated fetch", { fetchId, current: fetchIdRef.current });
        }
      } catch (err) {
        if (isMounted && fetchId === fetchIdRef.current) {
        ...
        if (isMounted && fetchId === fetchIdRef.current) {
          setLoading(false);
          console.log("fetchMessages: State", { loading: false, error, messages });
        }
      }
    }
    fetchMessages();
  ...
  return (
    ...
    >
      ...
      >
        ...
        <ScrollView
          ref={scrollViewRef}
          style={[styles.messagesContainer, { backgroundColor: isDark ? "#333" : "#eee" }]}
        ...
        >
          {loading ? (
            ...
            <View style={[styles.messages, { backgroundColor: isDark ? "#444" : "#ddd" }]}>
              {activeIntake === "current" ? (
                messages.map((msg) => (
                  <View
                    key={msg.id}
                    style={[styles.message, { backgroundColor: isDark ? "#555" : "#ccc" }]}
                  >
                    {msg.avatar && msg.avatar.startsWith("http") ? (
                      <Image
                        ...
                        onError={(e) => console.log("Image load error", msg.id, e.nativeEvent.error)}
                      />
                    ) : (
                      <View
                        style={[
                    ...
                    <View style={styles.messageContent}>
                      <View style={styles.messageHeader}>
                        <Text
                          style={{ ...styles.messageUser, color: isDark ? "#fff" : "#333" }}
                        >
                          {msg.user}
                        </Text>
                        {msg.isInstructor && (
                          ...
                          >
                            <Text
                              style={{ ...styles.badgeText, color: isDark ? "#a500ff" : "#7b00cc" }}
                            >
                              Instructor
                            </Text>
                          </View>
                        ...
                        <Text
                          style={{ ...styles.messageTime, color: isDark ? "#ccc" : "#666" }}
                        >
                          {msg.time}
                        </Text>
                      </View>
                      <Text
                        style={{ ...styles.messageText, color: isDark ? "#fff" : "#333" }}
                      >
                        {msg.message}
                      </Text>
                      <View style={styles.messageActions}>
                        ...
                        >
                          ...
                          />
                          <Text
                            style={{ ...styles.actionText, color: isDark ? "#ccc" : "#666" }}
                          >
                            {msg.likes}
                          </Text>
                        </TouchableOpacity>
                ...
                <>
                  ...
                  >
                    <Text
                      style={{ ...styles.archiveText, color: isDark ? "#ccc" : "#666" }}
                    >
