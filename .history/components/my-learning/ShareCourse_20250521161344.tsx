...
export default function ShareCourse({ course, progress }: ShareCourseProps) {
  ...
  const handleShare = async () => {
    ...
    try {
      const courseIdentifier = course.slug || course.id; // Fallback to id if slug is undefined
      if (!courseIdentifier) {
        setShareError("Course identifier is unavailable.");
        return;
      }
      const shareMessage = `Check out "${
        course.title
      }" on BrainiX! Learn at your own pace with ${
        course.totalLessons || 80
      } lessons. Join me: https://braini-x-one.vercel.app/courses/${courseIdentifier}`;
      const result = await Share.share({
        message: shareMessage,
        url: `https://braini-x-one.vercel.app/courses/${courseIdentifier}`,
  ...
  const courseIdentifier = course.slug || course.id || "unknown"; // Fallback for display
        ...
        <View style={styles.content}>
          ...
          >
            {course.shortDescription ||
              "Learn new skills with this amazing course!"}
          </Text>
          <Text
            style={[styles.url, { color: isDark ? "#a500ff" : "#7b00cc" }]}
            numberOfLines={1}
          >
            https://braini-x-one.vercel.app/courses/{courseIdentifier}
          </Text>
          <Text style={[styles.meta, { color: isDark ? "#ccc" : "#666" }]}>
            {course.totalLessons || 80} Lessons | {Math.round(progress)}%
            Complete
          </Text>
        </View>
